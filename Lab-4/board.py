...
class Board:
    ...
    def __init__(self, tiles: list[list[int]]):
        ...
        """
        self._tiles = [row[:] for row in tiles]  # Create a deep copy to ensure immutability
        self._n = len(tiles)
        
        # Find the blank position (0) for later use
        self._blank_row, self._blank_col = self._find_blank()
        
        # Pre-compute the manhattan and hamming distances
    ...
    def neighbors(self) -> list[Board]: # you might have to import annotations from __future__
        ...
        # Possible directions to move the blank: up, down, left, right
        directions = [(-1, 0), (1, 0), (0, -1), (0, 1)]
        
        for d_row, d_col in directions:
            new_row, new_col = self._blank_row + d_row, self._blank_col + d_col
            
            # Skip if the new position is outside the board
    ...
    def twin(self) -> Board:
        ...
        # Find the second non-blank tile
        for i in range(self._n):
            for j in range(self._n):
